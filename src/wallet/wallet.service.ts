...
import { NibiruService } from 'src/nibiru/nibiru.service';
import { newSignerFromMnemonic } from '@nibiruchain/nibijs';
...
export class WalletService {
  ...
  constructor(
    ...
    private readonly nibiruService: NibiruService,
  ) {
  ...
  async transferToUser(walletAddress: string, amount: number) {
    const txhash = await this.nibiruService.transferSplToken(
      walletAddress,
      amount,
    );
    return txhash;
  ...
  async getBalanceCommissionWallet() {
    ...
    const balance = await this.nibiruService.getSplTokenBalance(fromAddr);
    return balance;
  }
}
